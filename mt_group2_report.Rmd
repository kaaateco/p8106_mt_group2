...
kable(summary_table, digits = 4, caption = "Figure 11. Regression Coefficients for MLR Model") %>%
...
kable(summary_table, digits = 4, caption = "Figure 12. Regression Coefficients for MARS Model") %>%
  kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover"))
```
...
model_perf <- tibble(
  ...
  `Adjusted R-squared` = c(0.149, 0.048, 0.169),
...
model_perf %>%
  gt() %>%
  tab_header(
    title = "Table 5: Model Performance on Independent Test Set (dat2)"
  ) %>%
  cols_label(
    Model = "Model",
    RMSE = "RMSE ↓",
